...
-- CAST (value AS TYPE);				Явное преобразонвание значения (value) в желаемый тип (type;)
-- DATENAME(PART,date);					Возвращает название фрагмента даты (месяца, дня недели)
-- DATENAME(PART,date);					Возвращает числовое значение  фрагмента даты (месяца, дня недели)
-- DATEADD(UNIT,amout, date OR time);	Добавляет заданное количество (amount) единиц времени (UNIT) к дате либо времени (date OR time);
-- IFF (CONDITION, value_1, value_2);	Conditional Ternary operator: Если условие (CONDITION) вернуло TRUE, то IIF возвращает значение 'value_1', в противном случае - 'value_2'
